...
@section('title', __('sgb.new_main_objective') . ' - ' . __('sgb.sgb'))

@section('content')
    <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-6">{{ __('sgb.new_main_objective') }}</h2>
        ...
        <form action="{{ route('main-objectives.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="quarter_id" name="quarter_id"
                    ...
                    <option value="">{{ __('Selecione um Período') }}</option> {{-- Chave nova: sgb.select_quarter --}}
            ...
            <div class="mb-4">
                ...
                <select id="responsible_user_id" name="responsible_user_id"
                        class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    <option value="">{{ __('Selecione um Responsável') }}</option> {{-- Chave nova: sgb.select_responsible --}}
            ...
            <div class="mb-4">
                ...
                <select id="status" name="status" required
                        class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    @foreach ($statuses as $status) {{-- Assegure que $statuses é passado pelo controller --}}
                        <option value="{{ $status }}" {{ old('status', 'Aguardando Início') == $status ? 'selected' : '' }}> {{-- Default para 'Aguardando Início' --}}
                            {{ $status }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6"> {{-- Aumentei a margem inferior antes dos botões --}}
                ...
                <textarea id="notes" name="notes" rows="3"
                          class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end space-x-4">
                <a href="{{ route('main-objectives.index') }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    {{ __('sgb.cancel') }}
                </a>
                <button type="submit"
                    ...
                    {{ __('sgb.save') }} {{-- Ajustado para usar a chave 'sgb.save' apenas --}}
                </button>
            </div>
        </form>
    </div>
