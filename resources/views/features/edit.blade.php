...
@section('title', __('sgb.edit_feature') . ': ' . $feature->title . ' - ' . __('sgb.sgb'))
...
@section('content')
    <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-6">
            {{ __('sgb.edit_feature') }}: <span class="text-blue-600 dark:text-blue-400">{{ $feature->title }}</span>
        </h2>
        ...
        <form action="{{ route('features.update', $feature) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4"
                          class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200">{{ old('description', $feature->description) }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="objective_id" name="objective_id" required
                    ...
                    <option value="">{{ __('sgb.select_main_objective') }}</option>
                    @foreach ($mainObjectives as $objective) {{-- Assegure que $mainObjectives é passado pelo controller --}}
            ...
            <div class="flex items-center justify-end space-x-4">
                <a href="{{ route('features.show', $feature) }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    {{ __('sgb.cancel') }}
                </a>
                <button type="submit"
